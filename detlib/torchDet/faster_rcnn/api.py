import torch
from . import fasterrcnn_mobilenet_v3_large_320_fpn
...
class TorchFasterRCNN(DetectorBase):
    ...
    def load(self, model_weights=None, **args):
        ...
        self.detector = fasterrcnn_mobilenet_v3_large_320_fpn(pretrained=True,
                                                **kwargs) if model_weights is None else fasterrcnn_mobilenet_v3_large_320_fpn()
        self.detector = self.detector.to(self.device)
        self.detector.eval()
        self.detector.requires_grad_(False)
        print("Detector initialized and ready for use.")

    def __call__(self, batch_tensor, score='bbox', **kwargs):
        assert self.detector is not None, "The detector model is not initialized. Please call load() first."

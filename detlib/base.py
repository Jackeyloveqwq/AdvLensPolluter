...
class DetectorBase(ABC):
    # def __init__(self, name: str, cfg, input_tensor_height_size: int, input_tensor_width_size: int, device: torch.device):
    def __init__(self, name: str, cfg, input_tensor_size: int, device: torch.device):
        ...
        :param input_tensor_size: size of the tensor to input the detector. Square (x*x).一定要正方形吗
        :param device: torch device (cuda or cpu)
        """
        self.name = name
        self.device = device
        self.detector = None
        self.input_tensor_size = input_tensor_size  # 正方形图像输入
        # self.input_tensor_height_size = input_tensor_height_size
        # self.input_tensor_width_size = input_tensor_width_size
        self.cfg = cfg
    ...
    def load(self, model_weights: str, **args):
        """
                init the detector model, load trained model weights and target detection classes file, 80 classes in coco
                :param model_weights
                :param **args:
                    classes_path

...
def rescale_boxes(boxes, current_dim, original_shape):
    ...
    """
    curr_h, curr_w = current_dim
    orig_h, orig_w = original_shape
    # Scaling factors
    scale_h = orig_h / curr_h
    scale_w = orig_w / curr_w
    # Rescale bounding boxes
    boxes[:, 0] = boxes[:, 0] * scale_w  # x1
    boxes[:, 1] = boxes[:, 1] * scale_h  # y1
    boxes[:, 2] = boxes[:, 2] * scale_w  # x2
    boxes[:, 3] = boxes[:, 3] * scale_h  # y2
    return boxes
...
#     pad_x = max(orig_h - orig_w, 0) * (current_dim[1] / original_shape[1])
#     pad_y = max(orig_w - orig_h, 0) * (current_dim[0] / original_shape[0])
#
#     # Image height and width after padding is removed
#     unpad_h = current_dim[0] - pad_y
#     unpad_w = current_dim[1] - pad_x
#

...
class HHYolov2(DetectorBase):
    ...
    def __call__(self, batch_tensor, **kwargs):
        ...
        # print(bbox_array)
        # Adjust YOLOv2 output to match YOLOv5 format
        batch_size = detections_with_grad.size(0)
        num_classes = self.detector.num_classes
        detections_with_grad_view = detections_with_grad.permute(0, 2, 3, 1).contiguous().view(batch_size, -1,
                                                                                          4 + 1 + num_classes)

        # output = {'bbox_array': bbox_array, 'obj_confs': obj_confs, "cls_max_ids": cls_max_ids}
        output = {'bbox_array': bbox_array, 'output_after_model': detections_with_grad_view}
        return output
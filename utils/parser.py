...
class ConfigParser:
    ...
    def __init__(self, config_file):
        ...
        self.attack_list = []
        self.load_config()
        self.all_class_names = load_class_names(os.path.join(PROJECT_DIR, self.DATA.CLASS_NAME_FILE))  # 80 classes
        self.get_attack_list()
        self.num_classes = len(self.all_class_names)  # 类别总数

        # TODO: To be more universal. This is a temp var(Attack single cls).
        self.attack_cls = int(self.ATTACKER.ATTACK_CLASS)  # 一个临时变量，用于单类别攻击
    ...
    def rectify_class_list(self, class_str_list, dtype='int', split_str = ':'):
        ...
        if class_str_list == '-1':
            ...
            # attack seen classes(ATTACK_CLASS in cfg file)只能支持攻击"person"类别

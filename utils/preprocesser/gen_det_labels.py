...
class Utils:
    ...
    def save_label(self, preds, save_path, save_name, save_conf=True, rescale=True):
        ori_width_size = self.cfg.DETECTOR.INPUT_SIZE[1]
        ori_height_size = self.cfg.DETECTOR.INPUT_SIZE[0]
        ...
        for pred in preds:
            ...
            if rescale:
                pred[0] = pred[0] * ori_width_size
                pred[1] = pred[1] * ori_height_size
                pred[2] = pred[2] * ori_width_size
                pred[3] = pred[3] * ori_height_size
...
if __name__ == "__main__":
    ...
    parser.add_argument('-dr', '--data_root', type=str,
                        default=f"/home/ubuntu/T_camera_patch/data/physical_exp_dataset/blue_translucent_patch/images",
                        help="Image data dir path")
    parser.add_argument('-sr', '--save_root', type=str,
                        default=f"/home/ubuntu/T_camera_patch/data/physical_exp_dataset/blue_translucent_patch/labels",
                        help="Label data dir path")
    parser.add_argument('-cfg', '--config_file', type=str, default=f'my_demo.yaml',
                        help=".yaml config file, a relative path.")
    parser.add_argument('-k', '--keep_scale', action="store_true", default=False,
                        help="To keep value range of labels as [0, 1] if set keep_scale=True. Default: rescale to the input size.")
    # parser.add_argument('-i', '--imgs', default='True', help="To save imgs.")
    ...
    for detector in detectors:
        ...
        # detector.load()  # faster rcnn和ssd需要激活这一行
        ...
        for index, img_tensor_batch in enumerate(tqdm(data_loader)):
            ...
            if args.imgs:
                save_dir = f'/home/ubuntu/T_camera_patch/kitti_tiny_test_ori_results/{detector.name}'
                os.makedirs(save_dir, exist_ok=True)
                img_numpy_int8 = img_tensor_batch[0].cpu().numpy().transpose(1, 2, 0).astype(np.uint8).copy()
                # img_numpy_int8 = np.ascontiguousarray(img_numpy).astype(np.uint8)
                plot_boxes_cv2(img_numpy_int8, np.array(preds[0].cpu()), cfg.all_class_names,
